...
(uartCfg.rxTail >= uartCfg.rxHead) ? (uartCfg.rxTail - uartCfg.rxHead) : (HAL_UART_ISR_RX_MAX - uartCfg.rxHead + uartCfg.rxTail)

#define HAL_UART_ISR_TX_AVAIL() \
(uartCfg.txHead > uartCfg.txTail) ? (uartCfg.txHead - uartCfg.txTail - 1) : (HAL_UART_ISR_TX_MAX - uartCfg.txTail + uartCfg.txHead - 1)
